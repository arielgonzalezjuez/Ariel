clima_seco(P):-nl, write(P), write('Hay poca humedad en el ambiente? [s/n]: '), read(R), R=s.
cambio_de_temperatura(P):-nl, write(P), write('Se ansian cambios en la temperatura? [s/n]: '), read(R), R=s.
cielo_despejado(P):-nl, write(P), write('Se observa un cielo azul claro sin marejadas ni nubes oscuras? [s/n]: '), read(R), R=s.
...
vientos_concordantes(P):-nl, write(P), write('hay vientos concordantes? [s/n]: '), read(R), R=s.
concentracion_nubes(P):-nl, write(P), write('hay concentracion irregular de  nubes? [s/n]: '), read(R), R=s.
...
vientos_min(P):-nl, write(P), write('hay vientos menores a 60 km/h? [s/n]: '),read(R), R=s.
...
diagnostico(P):-nl, write('No se llego al objetivo de hallar un diagnostico de pronostico del tiempo ').